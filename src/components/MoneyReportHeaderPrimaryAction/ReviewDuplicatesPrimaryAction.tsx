    ...
    return (
        <Button
            ...
            onPress={() => {
                ...
                let shouldWaitForOptimisticThread = false;
                ...
                if (threadID && !transactionThreadReport && iouAction?.childReportID === threadID) {
                    setOptimisticTransactionThread(threadID, moneyRequestReport?.reportID, iouAction.reportActionID, moneyRequestReport?.policyID);
                    shouldWaitForOptimisticThread = true;
                }

                if (threadID) {
                    const route = createDynamicRoute(DYNAMIC_ROUTES.TRANSACTION_DUPLICATE_REVIEW.getRoute(threadID));
                    if (shouldWaitForOptimisticThread) {
