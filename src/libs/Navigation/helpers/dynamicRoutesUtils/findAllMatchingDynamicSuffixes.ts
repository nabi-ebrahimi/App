 ...
 */
function tryMatchParametric(candidate: string, candidateSegmentCount: number, patterns: CompiledEntry[]): DynamicSuffixMatch[] {
...
function findAllMatchingDynamicSuffixes(path = ''): DynamicSuffixMatch[] {
    ...
    for (let i = 0; i < segments.length; i++) {
        const matches = tryMatchParametric(segments.slice(i).join('/'), segments.length - i, compiledStrictParametricDynamicRoutes);
    ...
    for (let i = 0; i < segments.length; i++) {
        const matches = tryMatchParametric(segments.slice(i).join('/'), segments.length - i, compiledOptionalParametricDynamicRoutes);
        for (const match of matches) {
            if (seenPatterns.has(match.pattern)) {
                continue;
            }
